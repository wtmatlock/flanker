...
def get_arguments():
    ...
    cluster.add_argument(
        '-cl',
        '--cluster',
        help='Turn on clustering mode?',
        action='store_true'),
    cluster.add_argument(
        '-o',
        '--outfile',
        action='store',
        help='Prefix for the clustering file',
        default='out'),
    cluster.add_argument(
        '-tr',
        '--threshold',
        action='store',
        help='mash distance threshold for clustering',
        default="0.001"),
    cluster.add_argument(
        '-p',
        '--threads',
        action='store',
        help='threads for mash to use',
        default='1'),
    cluster.add_argument(
        '-k',
        '--kmer_length',
        action='store',
        help='kmer length for Mash',
        default='21'),
    cluster.add_argument(
        '-s',
        '--sketch_size',
...
def run_abricate(file):
    ...
    p = subprocess.Popen(
        ...
        stderr=subprocess.PIPE)  # run abricate
...
def flank_fasta_file_circ(file, window, gene):
    ...
    # name of abricate output for fasta
    unfiltered_abricate_file = file + '_resfinder'
    ...
    for guid in guids:
        ...
        if pos==True:
            ...
            pos = list(pos)
            gene_sense = abricate_file.loc[abricate_file['GENE'] == pos[2]].filter(items=[
                                                                                   'STRAND'])
            ...
            d = {(True,
                 ...
                 pos,
                 w,
                 l: record.seq[(pos[0] - w):(pos[1] + w)],
                 (True,
                  'upstream'): lambda record,
                 pos,
                 w,
                 l: record.seq[(pos[0] - w):(pos[1])],
                 (True,
                  'downstream'): lambda record,
                 pos,
                 w,
                 l: record.seq[pos[0]:(pos[1] + w)],
                 (False,
                  'both'): lambda record,
                 pos,
                 w,
                 l: record.seq[(pos[0] - w):pos[0]] + record.seq[pos[1]:(pos[1] + w)],
                 (False,
                  'upstream'): lambda record,
                 pos,
                 w,
                 l: record.seq[(pos[0] - w):pos[0]],
                 (False,
                  'downstream'): lambda record,
                 pos,
                 w,
                 l: record.seq[pos[1]:(pos[1] + w)]}

            d_before = {(True,
                         'both'): lambda record,
                        pos,
                        w,
                        l: record.seq[(l - (w - pos[0])):l] + record.seq[0:(pos[1] + w)],
                        (True,
                         'upstream'): lambda record,
                        pos,
                        w,
                        l: record.seq[(l - (w - pos[0])):l] + record.seq[0:(pos[1])],
                        (True,
                         'downstream'): lambda record,
                        pos,
                        w,
                        ...
                        pos,
                        w,
                        l: record.seq[(l - (w - pos[0])):l] + record.seq[0:pos[0]] + record.seq[pos[1]:(pos[1] + w)],
                        (False,
                         'upstream'): lambda record,
                        pos,
                        w,
                        l: record.seq[(l - (w - pos[0])):l] + record.seq[0:pos[0]],
                        (False,
                         'downstream'): lambda record,
                        pos,
                        w,
            ...
            d_after = {(True,
                       ...
                       pos,
                       w,
                       l: record.seq[(pos[0] - w):l] + record.seq[0:(pos[1] + w - l)],
                       (True,
                        'upstream'): lambda record,
                       pos,
                       w,
                       ...
                       pos,
                       w,
                       l: record.seq[(pos[0]):l] + record.seq[0:(pos[1] + w - l)],
                       (False,
                        'both'): lambda record,
                       pos,
                       w,
                       l: record.seq[(pos[0] - w):pos[0]] + record.seq[pos[1]:l] + record.seq[0:((pos[1] + w) - l)],
                       (False,
                        'upstream'): lambda record,
                       pos,
                       w,
                       ...
                       pos,
                       w,
                       l: record.seq[pos[1]:l] + record.seq[0:((pos[1] + w) - l)]}
            ...
            for record in SeqIO.parse(file, "fasta"):
                ...
                if record.id == guid:
                    ...
                    if w > 0.5 * (pos[0] - pos[1] + l):
                        log.info(
                            f"Error: Window length {w} too long for sequence length {l}")
                        continue

                    # if window exceeds sequence length after gene
                    if (pos[1] + w > l):
                        log.info("Window exceeds seq length after gene")
                        record.seq = d_after[(args.include_gene, x)](
                            record, pos, w, l)
                    ...
                    if (pos[0] - w < 0):
                        ...
                        record.seq = d_before[(args.include_gene, x)](
                            record, pos, w, l)
                        writer(record, pos[2], w, guid, args.flank, gene_sense)
                        continue
                        ...
                        record.seq = d[(args.include_gene, x)](
                            record, pos, w, l)
                        writer(record, pos[2], w, guid, args.flank, gene_sense)
                        continue
    ...
    for guid in guids:
        ...
        if pos==True:
            ...
            d_lin = {(True,
                     ...
                     w,
                     l: record.seq[max(0,
                                       pos[0] - w):pos[0]] + record.seq[pos[1]:min(l,
                                                                                   pos[1] + w)],
                     (False,
                      'upstream'): lambda record,
                     pos,
            ...
            for record in SeqIO.parse(file, "fasta"):
                ...
                if record.id == guid:
                    ...
                    record.seq = d_lin[(args.include_gene, x)](
                        record, pos, w, l)
                    writer(record, pos[2], w, guid, args.flank, gene_sense)
    ...
    if args.window_stop is not None:
        for i in range(args.window, args.window_stop, args.window_step):
            for gene in gene_list:
                ...
                if args.cluster and args.mode == 'default':

                    cluster.define_clusters(
                        gene,
                        i,
                        args.threads,
                        args.threshold,
        ...
        for gene in gene_list:
            if args.circ:
                flank_fasta_file_circ(
                    args.fasta_file, args.window, gene.strip())
            else:

                flank_fasta_file_lin(
                    args.fasta_file, args.window, gene.strip())
            if args.cluster and args.mode == 'default':
